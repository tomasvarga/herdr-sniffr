...
# tuicr :clear removes all drafts but prompts for confirmation → Enter twice
# (execute, then confirm). tuicr auto-refreshes on external `review add`.
clear_tuicr(){
  herdr pane send-text "$TU" ':clear' >/dev/null 2>&1; sleep 0.3
  herdr pane send-text "$TU" "$(printf '\r')" >/dev/null 2>&1; sleep 0.4
  herdr pane send-text "$TU" "$(printf '\r')" >/dev/null 2>&1; sleep 0.3
}
...
# start clean: tuicr session was reset pre-record; clear hunk (daemon persists)
sleep 1
